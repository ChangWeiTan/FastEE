...
import classifiers.nearestNeighbour.elasticDistance.lowerBounds.LbWdtw;
...
public class WDTW1NN extends OneNearestNeighbour {
    ...
    protected LbWdtw lbComputer = new LbWdtw();

    public WDTW1NN(final int paramId, final TrainOpts trainOpts) {
        this.g = 0;
        this.classifierIdentifier = "WDTW_1NN_R1";
...
//        this.setParamsFromParamId(paramId);
    ...
    public WDTW1NN(final int paramId, final TrainOpts trainOpts, final int useDerivative) {
    ...
    public double lowerBound(final Sequence candidate,
        ...
        if (refreshWeights) {
            initWeights(candidate.length());
        }

        // U and L are built on query
        return lbComputer.distance(candidate, weightVector[0], queryMax, queryMin);
    }
    ...
    public int predictWithLb(Sequence query) {
        ...
        for (int candidateIndex = 1; candidateIndex < trainData.size(); candidateIndex++) {
            ...
            dist = lowerBound(candidate, testCache.getMax(queryIndex), testCache.getMin(queryIndex));
            if (dist < bsfDistance) {
    ...
    public int classifyLoocvLB(final Sequence query, final int queryIndex) throws Exception {
        ...
        for (candidateIndex = nextIndex; candidateIndex < this.trainData.size(); candidateIndex++) {
            ...
            dist = lowerBound(candidate, trainCache.getMax(queryIndex), trainCache.getMin(queryIndex));
            if (dist < bsfDistance) {
