#!/bin/bash 
usage() { echo "Usage: $0 [-p <Dataset Name>] [-c <EE|LbEE|FastEE|ApproxEE>] [-d <Dataset Directory>] [-o <Output Directory>]" 1>&2; exit; }
...
while getopts p:c:d:o:h: option; do
      case "${option}" in
            p)    PROBLEM=${OPTARG};;
            c)    CLASSIFIER=${OPTARG};;
            d)    DATASETDIR=${OPTARG};;
...
for distance in "${DISTANCES[@]}"; do
      if [ $CLASSIFIER = "EE" ]; then
            ...
            java -Xmx14g -Xms14g -cp $PROJECTDIR/lib/*: experiments.IndividualClassifierApproxEE $OUTPUTDIR $DATASETDIR $PROBLEM $distance
      else
