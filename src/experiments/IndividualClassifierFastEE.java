 ...
 * <p>
 * Training an indiviual classifier using FastEE
 */
public class IndividualClassifierFastEE extends Experiments {
    private static String datasetName = "Car";
    private static String distanceMeasure = "LCSS";
    ...
    public static void main(String[] args) throws Exception {
        ...
        // display them
        System.out.println("[FAST-EE] Input arguments:");
        System.out.println(String.format("[FAST-EE] Output path     : %s", outputPath));
        System.out.println(String.format("[FAST-EE] Dataset path    : %s", datasetPath));
        System.out.println(String.format("[FAST-EE] Dataset name    : %s", datasetName));
        System.out.println(String.format("[FAST-EE] Distances       : %s", distanceMeasure));

        // load data
        System.out.println(String.format("[FAST-EE] Loading %s", datasetName));
        ...
        System.out.println(String.format("[FAST-EE] %s loaded", datasetName));
        ...
        switch (distanceMeasure) {
            ...
            case "ERP":
                ...
                break;
            default:
                throw new RuntimeException("[FAST-EE] Undefined distance measure for EE");
        }
        // initialise cache
        System.out.println(String.format("[FAST-EE] Caching %s", datasetName));
        ...
        System.out.println(String.format("[FAST-EE] %s cached", datasetName));

        // start building classifier
        System.out.println(String.format("[FAST-EE] Building NN-%s classifier", distanceMeasure));
        ...
        System.out.println(String.format("[FAST-EE] TRAINING NN-%s classifier", distanceMeasure));

        double[] cvAccAndPred = classifier.loocv(train);

        trainTime = classifier.getCvTime();
        System.out.println(String.format("[FAST-EE] Train Time: %.5f s, Accuracy: %.5f", trainTime, cvAccAndPred[0]));

        // save training results
        String trainRes = trainTime + "," + cvAccAndPred[0] + "," + classifier.getBsfParamId();
        OutFile of = new OutFile(outputPath + datasetName + "/" + datasetName + "_FastEE_" + distanceMeasure + "_TRAIN.csv");
        ...
        System.out.println(String.format("[FAST-EE] Test Time: %.5f s, Accuracy: %.5f", testTime, accuracy));

        // save classification results
        String testRes = testTime + "," + accuracy;
        of = new OutFile(outputPath + datasetName + "/" + datasetName + "_FastEE_" + distanceMeasure + "_TEST.csv");
